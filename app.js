...
form.addEventListener("submit", (e) => {
	//	document.write("praise JESUS!!!");
	//	console.log("praise JESUS!!!")
	e.preventDefault();
	//	console.log("praise JESUS!!!");
	const value = parseFloat(amount.value);
	//	console.log(typeof value);
	const random_value = Math.floor(Math.random() * text.length);

	if (isNaN(value) || value <= 0 || value > 9) {
		result.innerHTML = `<p class = "result">${text[random_value]}</p>`;
	} else {
		let temp_text = text.slice(0, value);
		//	result.innerHTML = `<p class = "result"> ${temp_text}             </p>`;
		temp_text = temp_text.map((item) => {
